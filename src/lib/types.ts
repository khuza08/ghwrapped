...
export interface GitHubWrappedData {
  ...
  commits: {
    commitsByDate: Record<string, number>; // date string -> commit count
  ...
  repositories: {
    ...
    topByStars: Array<GitHubRepo>;
    topByForks: Array<GitHubRepo>;
...
export interface GitHubWrappedSlide {
  ...
  title: string;
  subtitle?: string;
  component: React.ComponentType<{ data: GitHubWrappedData }>;
  icon?: string;
}