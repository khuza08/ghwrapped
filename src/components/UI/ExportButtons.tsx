...
const ExportButtons: React.FC<ExportButtonsProps> = ({ data }) => {
  ...
  const handleExportWrappedImage = () => {
    handleExportImage('[data-export-wrapped]', `github-wrapped-${user.login}-2024.png`);
  };

  const handleExportBanner = () => {
    handleExportImage('[data-export-banner]', `github-wrapped-${user.login}-2024-banner.png`);
  };
