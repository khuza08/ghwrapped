...
const SummarySlide: React.FC<WrappedSlideProps> = ({ data }) => {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto text-center">
      ...
      >
        <h3 className="text-2xl md:text-3xl font-bold text-gray-800 mb-2">GitHub Wrapped 2024</h3>
        <div className="flex items-center justify-center mb-4 md:mb-6">
          ...
          <div className="text-left">
            <h4 className="text-lg md:text-xl font-semibold text-gray-800">{user.name || user.login}</h4>
            <p className="text-sm md:text-base text-gray-600">@{user.login}</p>
          </div>
        </div>
      </motion.div>
      ...
      >
        ...
        <motion.div
          className="bg-yellow-50 p-3 md:p-4 rounded-lg"
          whileHover={{ scale: 1.03 }}
          transition={{ type: "spring", stiffness: 400, damping: 10 }}
        >
        ...
        <motion.div
          className="bg-purple-50 p-3 md:p-4 rounded-lg"
          whileHover={{ scale: 1.03 }}
          transition={{ type: "spring", stiffness: 400, damping: 10 }}
        >
          ...
          <div className="text-xs md:text-sm text-gray-600">On GitHub</div>
        </motion.div>
      </motion.div>

      <motion.div
        className="mt-4 md:mt-6 text-sm md:text-base text-gray-700"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
